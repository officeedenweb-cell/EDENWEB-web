...
export const JobCard = ({
  ...
  return <Card className="card-body bg-light d-md-flex justify-content-md-between align-items-md-center flex-md-row p-4 mb-3">
          ...
          <Col lg={9} className="mx-auto">
            <TabContainer defaultActiveKey={'0'}>
              <Nav className="nav-pills gap-1 nav-pills-light justify-content-center align-items-center mb-3 mb-sm-5">
                ...
                {jobs.map((job, idx) => <NavItem key={idx}>
                    <NavLink eventKey={`${idx}`}>{job}</NavLink>
                  </NavItem>)}
              </Nav>
              <TabContent>
                <TabPane eventKey={'0'} className="fade">
                  {allJobs.map((job, idx) => <JobCard job={job} key={idx} />)}
                </TabPane>
                <TabPane eventKey={'1'} className="fade">
                  {allJobs.slice(0, 2).map((job, idx) => <JobCard job={job} key={idx} />)}
                </TabPane>
                <TabPane eventKey={'2'} className="fade">
                  {allJobs.slice(2, 4).map((job, idx) => <JobCard job={job} key={idx} />)}
                </TabPane>
