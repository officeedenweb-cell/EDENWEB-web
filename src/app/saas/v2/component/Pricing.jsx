...
const Pricing = () => {
  ...
  return <section className="bg-dark position-relative price-wrap" data-bs-theme="dark">
      ...
      <Container className="position-relative">
        <Row className="g-4 g-lg-6">
          ...
          <Col md={6} xl={4}>
            {pricingData1.map(item => item.plans.map((plan, idx) => {
            if (item.duration === planDuration) return <PricingCard1 plan={plan} duration={item.duration} key={idx} />;
          }))}
          </Col>

          <Col md={6} xl={4}>
            {pricingData2.map(item => item.plans.map((plan, idx) => {
