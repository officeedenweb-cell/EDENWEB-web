...
const ProductCard = ({
  ...
  return <Card className="border bg-transparent overflow-hidden p-0 h-100">
      <div className="position-absolute start-0 top-0 p-3">
        ...
        {sale && <span className="badge text-bg-danger">
            {sale.discount}
            {sale.type === 'percent' && '%'} off
          </span>}
      </div>
      <CardHeader className="bg-light rounded m-2">
        {images.map((image, idx) => <Image alt="" src={image} key={idx} />)}
      </CardHeader>
      ...
      <CardBody className="pb-0">
        ...
        <ul className="list-inline">
          {review && Array(Math.floor(review.stars)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small">
          ...
          {review && review.stars < 5 && Array(5 - Math.ceil(review.stars)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small">
      ...
      <CardFooter className="bg-transparent d-flex justify-content-between align-items-center pt-0">
        ...
        {isInCart(product) ? <button className="btn btn-danger mb-0 z-index-2" onClick={() => updateQuantityForProduct(product, 0)}>
            <BsTrash />
          </button> : <button className="btn btn-dark mb-0 z-index-2" onClick={() => updateQuantityForProduct(product, 1)}>
            <BsCart />
          </button>}
      </CardFooter>
    </Card>;
