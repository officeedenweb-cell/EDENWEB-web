...
const TestimonialCard = ({
  ...
  return <Card className="card-body bg-light p-sm-5">
      ...
      <ul className="list-inline mb-4">
        {rating && Array(Math.floor(rating)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1">
                <BsFillStarFill size={16} className="text-warning" />
              </li>)}
        {!Number.isInteger(rating) && <BsStarHalf size={16} className="text-warning me-1" />}
        {rating && rating < 5 && Array(5 - Math.ceil(rating)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1">
